...
class ContractsApiTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_contracts_route_returns_503_when_database_disabled(self) -> None:
        response = self.client.get("/contracts")
        self.assertEqual(response.status_code, 503)
        payload = response.get_json() or {}
        self.assertFalse(payload.get("success", True))
        self.assertIn("Modulo de contratos indisponivel", str(payload.get("error", "")))

    def test_contract_reports_route_returns_503_when_database_disabled(self) -> None:
        response = self.client.get("/contracts/1/reports")
        self.assertEqual(response.status_code, 503)
        payload = response.get_json() or {}
        self.assertFalse(payload.get("success", True))
        self.assertIn("Modulo de relatorios indisponivel", str(payload.get("error", "")))
